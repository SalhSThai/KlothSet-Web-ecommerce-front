...
export default function AddNewProductBody(props) {
    const { close  } = props;
    const initialInfo = {
        productName: "",
        price: "", size: "",
    ...
    const submit = (e) => {e.preventDefault();;console.log('1');}
    ...
    return (
        <form className='grid grid-flow-row-dense grid-cols-4  justify-center items-start w-full p-4' onSubmit={e=>handleSubmit(e)}>
            ...
            <TextInputBar className={"col-span-2"} label="Price" id="PriceAdd" value={productInfo?.price} onChange={e => setProductInfo({ ...productInfo, price: e.target.value })} />
